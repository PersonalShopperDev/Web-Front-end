...
export default function StyleLists() {
  ...
  const maleStyleClick = (style: number) => {
    if (clickedMaleStyleList.includes(style)) {
      if (isMaleOverLength) setIsMaleOverLength(false)
      setClickedMaleStyleList(clickedMaleStyleList.filter((item) => item !== style))
    } else if (clickedMaleStyleList.length < 3) {
      setClickedMaleStyleList([...clickedMaleStyleList, style])
    } else {
  ...
  const femaleStyleClick = (style: number) => {
    if (clickedFemaleStyleList.includes(style)) {
      if (isFemaleOverLength) setIsFemaleOverLength(false)
      setClickedFemaleStyleList(clickedFemaleStyleList.filter((item) => item !== style))
    } else if (clickedFemaleStyleList.length < 3) {
      setClickedFemaleStyleList([...clickedFemaleStyleList, style])
    } else {
  ...
  const fetchStyleListData = async () => {
    ...
    setMaleStyleLists(list.male)
    setFemaleStyleLists(list.female)
  }
  useEffect(() => {
    fetchStyleListData()
  }, [])
